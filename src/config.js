...
module.exports = {
	name: "考试管理系统", // 项目的名字
	favicon: "./assets/imgs/exam.png", // 设置网页的favicon, 可以是外链, 也可以是本地
	footer: "考试管理系统", // footer中显示的字, 可以嵌入html标签
	...
	getAPIPath() {
		...
		if (this.isCrossDomain()) {
			...
			if (index < 0) paths.push("");
			else paths.push(tmp.substring(0, index + 1));
		} else {
			paths.push("");
		}
		...
		if (this.api.path) {
			...
			if (begin > end) paths.push("");
			else paths.push(tmp.substring(begin, end + 1));
		} else {
			paths.push("");
		}
