 ...
 * @LastEditTime: 2023-06-14 18:30:28
...
import {
	deleteTabkItemsCreator,
	getLoggerTableCreator,
	getTableDataCreator,
	setTableDataCreator,
	updateTableItemsCreator
} from "../../redux/Table";
...
class DBTable extends React.PureComponent {
	...
	async componentWillMount() {
		...
		this.tryFetchSchema(this.props, (res) => {
			...
			if (this.state.loadingSchema) {
				this.setState(
					{ loadingSchema: false, data: this.props.tableList },
					this.refresh
				);
			}
		});
	 ...
	 */
	componentDidMount() {
		const hide = message.loading("正在获取列表数据....", 0);
		this.state.tableName === "question"
			? store.dispatch(getTableDataCreator())
			: store.dispatch(getLoggerTableCreator());
		hide();
		message.success("列表数据获取成功");
		// 使用下面这种方式才能获取到数据
		store.subscribe(() => {
			this.setState({
				data: store.getState().Table.tableList
			});
		});
	...
	componentWillReceiveProps(nextProps) {
		...
		logger.debug(
			"receive new props and try to render, nextProps = %o",
			nextProps
		);
	...
	handleFormSubmit = async (queryObj) => {
		...
		// 这时查询条件已经变了, 要从第一页开始查
		store.dispatch(getTableDataCreator(queryObj));
	};
	// 编辑弹框
	tableEditor = async (item) => {
		store.dispatch(updateTableItemsCreator(item));
		message.success("数据编辑成功");
		this.props.getTableListSuccess();
	};
	// 新增数据
	tableInsertItems = async (item) => {
		store.dispatch(setTableDataCreator(item));
		store.dispatch(getTableDataCreator());
	};
	// 删除
	tableDeleteItems = async (id) => {
		const hide = message.loading("正在删除......", 0);
		await this.props.delTableDataSuccess(id);
		hide();
	...
	render() {
		...
		if (
			...
			(!this.notFirstRender || (!this.inited && !this.errorMsg))
		) {
			this.notFirstRender = true;
			return (
				<Spin
					tip="loading schema..."
					spinning={this.state.loadingSchema}
					delay={500}>
		...
		return (
			<Spin spinning={this.state.loadingSchema} delay={500}>
				...
				<InnerTable
					...
					tableConfig={this.tableConfig}
					tableName={this.tableName}
					updateTable={this.tableEditor}
					tableDeleteItems={this.tableDeleteItems}
					tableInsertItems={this.tableInsertItems}
				/>
				{/* 分页组件 */}
...
const mapStateToProps = (state) => {
	return {
		tableList: state.Table.tableList // 登录后的用户信息
	};
};

const mapDispatchToProps = (dispatch) => {
	return {
		getTableListSuccess: bindActionCreators(getTableDataCreator, dispatch), // loginSuccess事件比较特殊, 不只Login组件会触发, App组件也会触发
		setTableDataSuccess: bindActionCreators(setTableDataCreator, dispatch),
		updTableDataSuccess: bindActionCreators(updateTableItemsCreator, dispatch),
		delTableDataSuccess: bindActionCreators(deleteTabkItemsCreator, dispatch),
		getLogTableSuccess: bindActionCreators(getLoggerTableCreator, dispatch)
		// handleLogoutSuccess: bindActionCreators(logoutSuccessCreator, dispatch)
	};
};
...
export default connect(
	mapStateToProps,
	mapDispatchToProps
)((props) => <DBTable key={props.location.pathname} {...props} />); // 解决同一组件不更新问题
