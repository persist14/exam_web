...
// 请求接口
import { quesCreate } from "../../apis/ques";
...
class InnerTable extends React.PureComponent {
	...
	componentWillReceiveProps(nextProps) {
		...
		if (
			this.props.tableName !== nextProps.tableName ||
			!nextProps.tableLoading
		) {
	...
	parseTableSchema(props) {
		...
		// 判断操作列是否存在
		const hasOp = parseResult.tableSchema.find(({ key }) => key === "opetor");
		let addOp;
		if (hasOp) {
			parseResult.tableSchema.splice(parseResult.tableSchema.length - 1, 1);
		}
		// 不需要导出和首次加载需要push
		!this.props.tableConfig.notOp &&
			parseResult.tableSchema.push({
				key: "opetor",
				title: "操作",
				render: (text, record) => (
					<div>
						<Tag
							color="#2db7f5"
							onClick={() => {
								this.tableEditor(record);
							}}>
							<Icon type="edit" />
							编辑
						</Tag>
						<Popconfirm
							onConfirm={() => this.deleteItems(record._id)}
							title="确定要删除吗?">
							<Tag color="pink">
								<Icon type="delete" />
								删除
		...
		this.tableSchema = InnerTableRenderUtils.bindRender(
			parseResult.tableSchema,
			tableName,
			this
		);
	}
	...
	tableEditor = (item) => {
		...
		this.setState(
			...
			() => this.setFormData({ ...item, id: item.key + 1 })
		);
	};
	// 删除
	deleteItems = (id) => {
		this.props.tableDeleteItems(id);
	};
	/**
	...
	onClickUpdate = (e) => {
		...
		if (!multiSelected) {
			...
			logger.debug(
				"update multiple records, keys = %s",
				newData[this.primaryKey]
			);
		}
		...
		if (multiSelected) {
			this.setState(
				{ modalVisible: true, modalTitle: "批量更新", modalInsert: false },
				() => this.setFormData(newData)
			);
		} else {
			this.setState(
				{ modalVisible: true, modalTitle: "更新", modalInsert: false },
				() => this.setFormData(newData)
			);
		}
	};
	...
	onClickDelete = (e) => {
		...
		Modal.confirm({
			title:
				this.state.selectedRowKeys.length > 1 ? "确认批量删除" : "确认删除",
	...
	handleModalOk = async () => {
		// 提交表单之前, 要先校验下数据
		let validated = true;
		this.formComponent.validateFieldsAndScroll(
			(err, values) => (validated = err ? false : validated)
		); // 不知道有没有更好的办法
		if (!validated) {
		...
		for (const key in oldObj) {
			...
			if (key === this.primaryKey && typeof oldObj[key] === "string") {
				// 我在InnerTableSchemaUtils限制死了, modal中的主键字段必定是个string
		...
		this.props.tableInsertItems(newObj);
		hide();
	};
	...
	// 更新数据
	updateItems = () => {
		const data = this.formComponent.getFieldsValue();
		this.props.updateTable({ ...data, id: this.state.selectItemsId });
		this.setState({
			modalVisible: false
		});
	};
		...
		return (
			<div>
				<div className="db-table-button">
					<Affix
						offsetTop={8}
						target={() => document.getElementById("main-content-div")}>
