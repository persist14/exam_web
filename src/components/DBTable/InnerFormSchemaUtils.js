...
const SchemaUtils = {
  ...
  getForm(tableName, schema, handleSubmit, handleReset) {
    // 是否要忽略缓存
    ...
    if (formMap.has(tableName)) {
      ...
      const newForm = this.createForm(tableName, schema, handleSubmit, handleReset);
      if (!ignoreCache) {
   ...
   * @returns {*}
   */
  createForm(tableName, schema, handleSubmit, handleReset) {
    const ignoreCache = TableUtils.shouldIgnoreSchemaCache(tableName);
    ...
    const tmpComponent = React.createClass({
      componentWillMount() {
        ...
        const schemaCallback = that.parse(schema, handleSubmit, handleReset);
        if (!ignoreCache) {
   ...
   */
  parse(schema, handleSubmit, handleReset) {
    ...
    return (getFieldDecorator) => {
      ...
      for (let i = 0; i < rows.length; i++) {
        ...
        formRows.push(<Row key={i} gutter={16}>
          {formCols}
          <Col span={6}  style={{ textAlign: 'left' }}>
            <Button type="primary" onClick={handleSubmit}><Icon type="search"/>查询</Button>
            <Button onClick={handleReset}><Icon type="cross"/>清除条件</Button>
          </Col>
        </Row>);
      }
