...
class App extends React.Component {
	...
	parseTabTitle() {
		...
		const addItem = (item) => {
			...
			if (item.icon) {
				...
				tabTitleMap.set(
					item.key,
					<span className="ant-layout-tab-text">{item.name}</span>
				);
			}
		};
	...
	onTabRemove = (targetKey) => {
		...
		// 过滤panes
		const newTabPanes = this.state.tabPanes.filter(
		...
		return (
			<div className="ant-layout-base">
				...
				<div
					id="main-content-div"
					className={
						this.props.collapse ? "ant-layout-main-collapse" : "ant-layout-main"
					}>
...
const mapStateToProps = (state) => {
	return {
		...
		user: state.Login.user // 登录后的用户信息
	};
};
